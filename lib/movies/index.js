...
async function cleanMovies(requests) {
  ...
  const overseerrMovieRequests = requests.filter(r => r.type === 'movie' && r.media.status === 5).sort((a, b) => {
    return new Date(a.media.mediaAddedAt) - new Date(b.media.mediaAddedAt)
  })
  const overseerrRequest = overseerrMovieRequests[0]
  const overseerrMovie = await getOverseerrMovieDetails(overseerrRequest.media.tmdbId)
